/**
 * BLKOUT IVOR AI Backend - Production Ready
...
// Chat endpoint with pathway context support
...
// Pathway integration endpoint
app.post('/api/pathway-context', (req, res) => {
  try {
    ...
    // Store pathway context for user (in production, this would use a database)
    ...
    console.error('Pathway Context Error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to save pathway context'
    });
  }
});
...
app.use('*', (req, res) => {
  res.status(404).json({
    ...
    available_endpoints: ['/health', '/api/chat', '/api/pathway-context']
  });
});
...
app.use((error, req, res, next) => {
  ...
  res.status(500).json({
    ...
    message: 'IVOR AI backend encountered an unexpected error'
  });
});

const port = process.env.PORT || 3001;
app.listen(port, () => {
  console.log(`🤖 IVOR AI Backend running on port ${port}`);
  console.log(`🏳️‍🌈 Supporting Black queer liberation through AI`);
});
