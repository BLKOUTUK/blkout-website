-- IVOR Community Liberation Platform - Database Setup Script
-- Execute this script in Supabase SQL Editor to set up journey-aware architecture
...
DROP TABLE IF EXISTS resource_coordination CASCADE;
DROP TABLE IF EXISTS cross_domain_events CASCADE;
DROP TABLE IF EXISTS organizing_projects CASCADE;
DROP TABLE IF EXISTS social_growth_metrics CASCADE;
...
CREATE TABLE social_growth_metrics (
    id SERIAL PRIMARY KEY,
...
CREATE TABLE organizing_projects (
    id SERIAL PRIMARY KEY,
...
CREATE INDEX idx_knowledge_content_embedding ON knowledge_content USING ivfflat (embedding_vector vector_cosine_ops);
CREATE INDEX idx_knowledge_content_domain_stage ON knowledge_content(domain, journey_stage);
CREATE INDEX idx_user_journeys_session ON user_journeys(session_id);
CREATE INDEX idx_user_journeys_domain_stage ON user_journeys(domain, current_stage);
CREATE INDEX idx_community_insights_type_domain ON community_insights(insight_type, domain);
CREATE INDEX idx_social_growth_platform_stage ON social_growth_metrics(platform, journey_stage);
CREATE INDEX idx_organizing_projects_type_stage ON organizing_projects(project_type, organizing_stage);
CREATE INDEX idx_cross_domain_events_type_status ON cross_domain_events(event_type, processing_status);
CREATE INDEX idx_resource_coordination_type_status ON resource_coordination(resource_type, availability_status);
CREATE INDEX idx_achievement_milestones_session_domain ON achievement_milestones(session_id, domain);
...
(
    'Trauma-Informed Healing Circles for Black Trans Community',
    'Guide for facilitating healing circles specifically designed for Black trans community members. Includes trauma-informed practices, cultural healing traditions, community accountability frameworks, and safety protocols.',
    'core',
    'community_healing',
    ARRAY['collective_healing', 'peer_support', 'trauma_recovery'],
    'community_healing_traditions',
    ARRAY['collective_wellness', 'peer_leadership', 'healing_justice']
),
(
    'Community Data Sovereignty and Privacy Protection',
    'Framework for protecting community data and ensuring democratic control over information collected about Black QTIPOC communities. Includes privacy protocols, consent processes, and community governance structures.',
    'community',
    'strategic_analysis',
    ARRAY['data_protection', 'community_control', 'privacy_sovereignty'],
    'liberation_strategy',
    ARRAY['systemic_understanding', 'strategic_planning', 'data_sovereignty']
);

-- Success message
SELECT 'IVOR Community Liberation Platform database setup completed successfully!' AS message;