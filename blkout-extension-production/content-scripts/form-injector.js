        ...
        <div class="blkout-form-container">
          ...
          <form id="blkout-quick-form" class="blkout-form">
            ...
            <div class="blkout-detected-info">
              ...
              ${contentData?.url ? `<small>🔗 Source: ${new URL(contentData.url).hostname}</small>` : ''}
              ${contentData?.moreInfoUrl && contentData.moreInfoUrl !== contentData.url ? 
                `<small>ℹ️ <a href="${contentData.moreInfoUrl}" target="_blank">More Details</a></small>` : ''}
              ${contentData?.registrationLink ? 
                `<small>📝 <a href="${contentData.registrationLink}" target="_blank">Register</a></small>` : ''}
            </div>
  ...
  async handleSubmission(overlay) {
    ...
    try {
      ...
      if (type === 'event') {
        ...
        // Add scraped metadata for events
        if (contentData?.moreInfoUrl) {
          submitData.moreInfoUrl = contentData.moreInfoUrl;
        }
        if (contentData?.organizer) {
          submitData.organizer = contentData.organizer;
        }
        if (contentData?.ticketInfo) {
          submitData.ticketInfo = contentData.ticketInfo;
        }
        if (contentData?.attendeeCount) {
          submitData.attendeeCount = contentData.attendeeCount;
        }
        if (contentData?.registrationLink) {
          submitData.registrationLink = contentData.registrationLink;
        }
      } else {
        ...
        if (contentData?.publishDate) {
          submitData.publishDate = contentData.publishDate;
        }
        if (contentData?.section) {
          submitData.section = contentData.section;
        }
      }
