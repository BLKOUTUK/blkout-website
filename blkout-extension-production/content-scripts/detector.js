...
class BlkoutContentDetector {
  ...
  detectEventbriteEvent() {
    ...
    // Scrape additional event details
    const organizer = document.querySelector('[data-automation="event-organizer-name"]')?.textContent?.trim() ||
                     document.querySelector('.organizer-name')?.textContent?.trim() ||
                     'Eventbrite Organizer';
    
    const ticketInfo = document.querySelector('[data-automation="ticket-price"]')?.textContent?.trim() ||
                      document.querySelector('.ticket-price')?.textContent?.trim();
    
    const eventUrl = window.location.href;
    const moreInfoUrl = eventUrl; // Use current URL as canonical link
  ...
  detectFacebookEvent() {
    ...
    // Scrape Facebook event details
    const dateTime = document.querySelector('[data-testid="event-date"]')?.textContent?.trim() ||
                     document.querySelector('.event-date')?.textContent?.trim();
    
    const location = document.querySelector('[data-testid="event-location"]')?.textContent?.trim() ||
                    document.querySelector('.event-location')?.textContent?.trim();
    
    const organizer = document.querySelector('[data-testid="event-host"]')?.textContent?.trim() ||
                     document.querySelector('.event-organizer')?.textContent?.trim() ||
                     'Facebook Event Host';
    
    const attendeeCount = document.querySelector('[data-testid="event-going-count"]')?.textContent?.trim();
  ...
  detectMeetupEvent() {
    ...
    // Scrape Meetup event details
    const dateTime = document.querySelector('[data-testid="event-time-display"]')?.textContent?.trim() ||
                     document.querySelector('.event-time')?.textContent?.trim();
    
    const location = document.querySelector('[data-testid="event-location"]')?.textContent?.trim() ||
                    document.querySelector('.venue-name')?.textContent?.trim() ||
                    document.querySelector('.event-venue')?.textContent?.trim();
    
    const organizer = document.querySelector('[data-testid="organizer-name"]')?.textContent?.trim() ||
                     document.querySelector('.group-name')?.textContent?.trim() ||
                     'Meetup Organizer';
    
    const attendeeCount = document.querySelector('[data-testid="attendee-count"]')?.textContent?.trim() ||
                         document.querySelector('.attendees-count')?.textContent?.trim();
  ...
  detectGuardianArticle() {
    ...
    // Scrape Guardian article metadata
    const publishDate = document.querySelector('time')?.getAttribute('datetime') ||
                       document.querySelector('[data-gu-name="meta"]')?.textContent?.trim();
    
    const section = document.querySelector('.badge-slot')?.textContent?.trim() ||
                   document.querySelector('[data-gu-name="section"]')?.textContent?.trim();
    
    const articleUrl = window.location.href;
    const moreInfoUrl = articleUrl; // Use current URL as canonical link
  ...
  detectGenericContent() {
    ...
    if (title && content && content.length > 200) {
      ...
      // Scrape generic metadata
      const author = document.querySelector('[name="author"]')?.getAttribute('content') ||
                    document.querySelector('.author')?.textContent?.trim() ||
                    document.querySelector('.byline')?.textContent?.trim();
      
      const publishDate = document.querySelector('[name="publish-date"]')?.getAttribute('content') ||
      ...
      // Try to find contact/registration links for events
      const registrationLink = document.querySelector('a[href*="register"]')?.href ||
                              document.querySelector('a[href*="signup"]')?.href ||
                              document.querySelector('a[href*="ticket"]')?.href;
      
      const currentUrl = window.location.href;
      const moreInfoUrl = registrationLink || currentUrl; // Use registration link if available, otherwise current URL
      ...
      this.foundContent = {
        ...
        author: author || (isEvent ? 'Community Submitted' : 'Web Article'),
        publishDate,
        category: isEvent ? 'Community' : 'community-response',
        url: currentUrl,
        moreInfoUrl: moreInfoUrl,
        registrationLink: isEvent ? registrationLink : null
      };
    }
  }
